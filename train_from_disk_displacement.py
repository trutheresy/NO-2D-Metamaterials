"""
Disk-backed I3O4 training: targets are channels 1–4 of outputs.pt (displacements only).
Standalone entrypoint; same training flow as train_from_disk.py except output channels and data sources.

Default batch / DataLoader settings match train_from_disk.py (I3O5 L1 disk run tuned on Windows).
"""
...
class DisplacementShardedDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    """Disk-backed dataset: inputs.pt; targets are outputs.pt channels 1–4 (displacement stack)."""

    def __init__(self, shards: list[DispShardInfo]):
        if not shards:
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        assert self._loaded_ridx is not None
        x = self._loaded_inputs[local_idx]
        y = self._loaded_outputs[local_idx, 1:5]
        return x, y
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Disk-backed I3O4 (displacement-only) training with local file logging.")
...
def discover_displacement_shards(output_root: Path, prefixes: tuple[str, ...]) -> list[DispShardInfo]:
    shards: list[DispShardInfo] = []
    ...
    for d in ds_dirs:
        ...
        if not validated_contract:
            ...
            if tuple(y.shape[1:]) != (5, 32, 32):
                raise ValueError(f"Invalid outputs shape in {pt}: {tuple(y.shape)} (need 5 ch to slice 1:5)")
            if int(y.shape[0]) != n:
                raise ValueError(
...
def displacement_dataset_version_hash(shards: list[DispShardInfo]) -> str:
    h = hashlib.sha256()
    h.update(b"i3o4_displacement_v1")
    for s in shards:
...
def build_run_name(args: argparse.Namespace) -> str:
    ...
    return (
        f"NO_I3O4_DISP_BCF16_{loss_tag}_HC{args.hidden_channels}_"
...
def maybe_cap_shards(shards: list[DispShardInfo], cap: int) -> list[DispShardInfo]:
    if cap <= 0:
        return shards
    out: list[DispShardInfo] = []
    remaining = cap
    for s in shards:
        if remaining <= 0:
            break
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("train_from_disk_displacement")
...
def main() -> None:
    ...
    try:
        ...
        train_shards = discover_displacement_shards(output_root, TRAIN_PREFIXES)
        test_shards = discover_displacement_shards(output_root, TEST_PREFIXES)
        ...
        train_ds = DisplacementShardedDataset(train_shards)
        test_ds = DisplacementShardedDataset(test_shards)
        ...
        data_ver = displacement_dataset_version_hash(train_shards + test_shards)
        params: dict[str, Any] = {
            "model_name": "FNO2d",
            "in_channels": 3,
            "out_channels": OUT_CHANNELS,
            "target_source": "outputs.pt channels 1–4 (displacements)",
            "hidden_channels": args.hidden_channels,
            "layers": args.layers,
            "modes_height": args.modes_height,
            "modes_width": args.modes_width,
