...
def main(cli_data_fn=None):
    ...
    # Create output directory with timestamp in root folder
    from datetime import datetime
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    # Get root directory (assume the script is in 2d-dispersion-py subfolder)
    root_dir = Path(__file__).parent.parent
    output_dir = root_dir / f'dispersion_plots_{timestamp}_mat'
    ...
    for struct_idx in struct_idxs:
        ...
        if 'EIGENVECTOR_DATA' in data and 'K_DATA' in data and 'M_DATA' in data and 'T_DATA' in data:
            ...
            print("  No eigenvector/K/M/T data available for reconstruction")
        ...
        plot_dispersion_on_contour(ax_disp, contour_info, frequencies_contour, contour_param,
                                   title=f'Dispersion Relation (Original, {mode_str})', mark_points=mark_points)
        ...
        if frequencies_recon is not None:
            ...
            plot_dispersion_on_contour(ax_recon, contour_info, frequencies_recon_contour, recon_param_to_use,
                                      title='Dispersion Relation (Reconstructed from Eigenvectors)', mark_points=mark_points)
