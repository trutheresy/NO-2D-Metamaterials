"""
Plot Dispersion Script for NumPy Dataset Format

This script loads NumPy format datasets (like set_br_1200n) and creates dispersion visualizations:
- Unit cell designs
- Dispersion relations along IBZ contours
- Material property fields

Supports both reduced (randomly downsampled) and unreduced datasets.
For reduced datasets, can optionally merge with original eigenvalue data.
"""
...
def main(cli_data_dir=None, cli_original_dir=None, n_structs=None):
    ...
    """
    print("=" * 70)
    print("Plot Dispersion Script (NumPy Dataset Format)")
    print("=" * 70)
    ...
    if cli_data_dir is not None:
        ...
        # Default to set_br_1200n
        data_dir = Path(r"D:\Research\NO-2D-Metamaterials\data\set_br_1200n")
    ...
    if not data_dir.exists():
        ...
        print("Please provide a valid path to the NumPy dataset.")
        return
    ...
    data = load_np_dataset(data_dir, original_data_dir)
    ...
    # Create output directory: plots/<dataset_name>_np/ in current directory
    import os
    current_dir = Path(os.getcwd())
    dataset_name = data_dir.name
    output_dir = current_dir / 'plots' / f'{dataset_name}_np'
    ...
    if 'design_params' in data:
        # If available, could extract from params
        pass
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot dispersion from NumPy dataset")
    parser.add_argument("data_dir", nargs="?", help="Path to the NumPy dataset directory")
    parser.add_argument("original_dir", nargs="?", help="Path to original full dataset (for reduced datasets)")
