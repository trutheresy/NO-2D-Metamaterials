# Regenerate no_degenerate_pivot percentile sample plots with manuscript/paper styling.
# Reuses existing per_sample_loss_*.npy from *_no_degenerate_pivot (does not recompute losses).
$ErrorActionPreference = "Stop"
$root = "d:\Research\NO-2D-Metamaterials"
Set-Location $root
$PY = "C:\ProgramData\anaconda3\envs\NO_2D_Metamaterials\python.exe"
$logPath = Join-Path $root "INFERENCE\_run_no_degenerate_pivot_sample_cases_manuscript.log"
...
Write-Log "=== no_degenerate_pivot sample cases manuscript regen ==="
Write-Log "Output subdirs: <LOSS>_sample_case_plots_no_degenerate_pivot_manuscript"
Write-Log "Models: $($models.Count); datasets: c_test,b_test; losses: $($losses -join ',')"
...
foreach ($MODEL in $models) {
    ...
    foreach ($d in $datasets) {
        ...
        foreach ($loss in $losses) {
            $srcSubdir = "$($loss.ToUpper())_sample_case_plots_no_degenerate_pivot"
            ...
            if (-not (Test-Path -LiteralPath $npy)) {
                # Fallback to staged per_sample_loss folder from the original backfill.
                $npy = Join-Path $PLOT "$tag\per_sample_loss_no_degenerate_pivot\per_sample_loss_${loss}_$tag.npy"
            }
            if (-not (Test-Path -LiteralPath $npy)) {
                throw "Missing loss array for $MODEL/$tag/$loss"
            }

            Write-Log "`n========== $tag sample_cases $loss -> $outSubdir =========="
            Run-Py "sample_cases_${loss}_no_deg_pivot_ms_${MODEL}_$tag" @(
...
Write-Log "`nALL_NO_DEGENERATE_PIVOT_SAMPLE_CASES_MANUSCRIPT_DONE $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
