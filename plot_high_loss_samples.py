...
Motivation: the NMAE/NMSE histograms have a few samples with enormous normalized loss.
Normalized losses divide by the mean magnitude of the *true* eigenfrequency field
(channel 0), so samples whose true field is near zero get a tiny denominator and an
inflated normalized loss. This script surfaces those samples and reports the denominator
so the pattern is explicit.
...
For each loss array it:
  ...
  - prints a per-sample table (loss, raw MAE/MSE, mean|t|, mean t^2, max|t| of the true ch0
    field) and an aggregate pattern summary over the top --analyze-top samples (band counts,
    repeated geometries, denominator distribution).
...
def main() -> None:
    ...
    p.add_argument("--channel", type=int, default=0, help="Channel the loss was computed on (default 0 = eigenfrequency).")
    ...
    p.add_argument("--show-eigfreq", action=argparse.BooleanOptionalAction, default=True,
                   help="Show the eigenfrequency channel column (default on here, since the normalized loss is computed on it).")
    args = p.parse_args()
    ...
    def truth_field_stats(d: int, w: int, b: int):
        t = eigenfrequency_uniform[d, w, b].float()
        return float(t.abs().mean()), float(t.square().mean()), float(t.abs().max())
    ...
    for loss_name, loss_path in args.loss_array:
        ...
        for rank in range(min(args.top, n)):
            ...
            err = out[args.channel] - tgt[args.channel]
            mae = float(err.abs().mean()); mse = float(err.square().mean())
            ma, ms, mx = truth_field_stats(d, w, b)
        ...
        denom_abs = np.array([truth_field_stats(int(arr[r, 1]), int(arr[r, 2]), int(arr[r, 3]))[0] for r in top_rows])
        ...
        print(f"  true-field mean|t| over worst {k}: median={np.median(denom_abs):.3e}  "
              f"mean={denom_abs.mean():.3e}  max={denom_abs.max():.3e}")
        # compare to dataset-wide typical mean|t|
        print(f"    (worst-{k} median mean|t| vs whole-array context: smaller => near-zero true field drives the blow-up)")
