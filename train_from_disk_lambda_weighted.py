...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default="l1",
        help="Per-channel base loss for weighted objective: l2 (MSE), l1 (MAE), or smoothl1 (Huber).",
    )
...
def build_run_name(args: argparse.Namespace) -> str:
    ...
    loss_tag = {"l2": "L2", "l1": "L1", "smoothl1": "SL1"}[args.loss]
    return (
        f"NO_I3O5W_BCF16_{loss_tag}_HC{args.hidden_channels}_"
        f"LR{args.learning_rate:.0e}_WD{args.weight_decay:.0e}_"
        f"SS{args.step_size}_G{args.gamma:.0e}_LE{LAMBDA_EIGEN:.0e}_{ch0_tag}_{ds}"
    )
...
def per_channel_reduction(pred: torch.Tensor, yb: torch.Tensor, base_loss: str) -> torch.Tensor:
    """
    Per-channel reduced error for weighted objective.
    - l2: mean squared error per channel
    - l1: mean absolute error per channel
    - smoothl1: smooth L1 (Huber) per channel
    """
    pf, yf = pred.float(), yb.float()
    if base_loss == "l2":
        return (pf - yf).square().mean(dim=(0, 2, 3))
    if base_loss == "l1":
        return (pf - yf).abs().mean(dim=(0, 2, 3))
    if base_loss == "smoothl1":
        return F.smooth_l1_loss(pf, yf, reduction="none", beta=1e-5).mean(dim=(0, 2, 3))
    raise ValueError(f"Unknown base_loss: {base_loss!r}")


def weighted_i3o5_loss(
    pred: torch.Tensor,
    yb: torch.Tensor,
    *,
    lambda_eigen: float = LAMBDA_EIGEN,
    base_loss: str = "l1",
) -> torch.Tensor:
    """
    Weighted objective:
      lambda * err(ch0 eigenfrequency) + (1-lambda) * sum(err(ch1..4 displacement)),
    where err is the per-channel reduction chosen by base_loss (l2, l1, or smoothl1).
    """
    ch_err = per_channel_reduction(pred, yb, base_loss=base_loss)
    eigen = ch_err[0]
    # Requested behavior: sum displacement channels rather than mean.
    disp = ch_err[1:].sum()
    return float(lambda_eigen) * eigen + (1.0 - float(lambda_eigen)) * disp
    ...
    with torch.no_grad():
        for xb, yb in loader:
            ...
            with amp_context(device, amp_mode):
                pred = model(xb)
                loss = weighted_i3o5_loss(pred, yb, lambda_eigen=lambda_eigen, base_loss=base_loss)
                loss_compare = (
                    weighted_i3o5_loss(pred, yb, lambda_eigen=lambda_eigen, base_loss=compare_base_loss)
                    if compare_base_loss is not None
                    else None
                )
            bs = xb.shape[0]
...
def _infer_previous_base_loss_weighted(run_dir: Path) -> str | None:
    """Return prior run's --loss (l1/l2/smoothl1) for weighted per-channel reductions; not nn.MSELoss names."""
    rc_path = run_dir / "resolved_config.json"
    if rc_path.is_file():
        try:
            rc = json.loads(rc_path.read_text(encoding="utf-8"))
            loss = str(rc.get("args", {}).get("loss", "")).lower()
            if loss in {"l1", "l2", "smoothl1"}:
                return loss
        except Exception:
            pass
    train_log = run_dir / "train.log"
    if train_log.is_file():
        loss_pat = re.compile(r"\bbase_loss=([a-zA-Z0-9_]+)")
        try:
            ...
            if last_match in {"l1", "l2", "smoothl1"}:
                return last_match
        except Exception:
            pass
    return None
...
def main() -> None:
    ...
    try:
        ...
        inferred_prev_base = _infer_previous_base_loss_weighted(run_dir) if resume_mode else None
        compare_base_loss_name: str | None = (
            inferred_prev_base if inferred_prev_base and inferred_prev_base != args.loss else None
        )
        if compare_base_loss_name is not None:
            logger.info(
                "Dual-loss logging enabled | active_base_loss=%s compare_base_loss=%s",
                args.loss,
                compare_base_loss_name,
            )
        ...
        params: dict[str, Any] = {
            ...
            "loss": "weighted_i3o5",
            "base_loss": args.loss,
            "lambda_eigen": LAMBDA_EIGEN,
            "lambda_displacement": 1.0 - LAMBDA_EIGEN,
        ...
        dual_compare = compare_base_loss_name is not None
        ...
        with metrics_csv.open(csv_mode, newline="", encoding="utf-8") as csv_f, metrics_jsonl.open("a", encoding="utf-8") as jsonl_f:
            ...
            for epoch in range(start_epoch, total_epochs + 1):
                    ...
                    with tqdm(
                        ...
                        for batch_idx, (xb, yb) in enumerate(train_loader, start=1):
                            ...
                            with amp_context(device, args.amp):
                                pred = model(xb)
                                loss = weighted_i3o5_loss(pred, yb, lambda_eigen=LAMBDA_EIGEN, base_loss=args.loss)
                                loss_compare = (
                                    weighted_i3o5_loss(
                                        pred, yb, lambda_eigen=LAMBDA_EIGEN, base_loss=compare_base_loss_name
                                    )
                                    if compare_base_loss_name is not None
                                    else None
                                )
                ...
                val_loss, val_loss_compare, val_ch, val_ch_cmp, val_sps = evaluate(
                    ...
                    lambda_eigen=LAMBDA_EIGEN,
                    base_loss=args.loss,
                    compare_base_loss=compare_base_loss_name,
                )
                ...
                if (
                    ...
                    and compare_base_loss_name is not None
                ):
                    emit_progress(
                        f"epoch={epoch}/{total_epochs} compare_weighted(base={compare_base_loss_name}) "
                        f"train={train_loss_compare:.6e} val={val_loss_compare:.6e}",
                        use_tqdm=use_tqdm,
                    )
                logger.info(
                    "epoch=%d/%d base_loss=%s train_ch_%s=%s val_ch_%s=%s train_loss=%.6e val_loss=%.6e lr=%.3e train_sps=%.1f",
                    epoch,
                    total_epochs,
                    args.loss,
                    args.loss,
                ...
                if (
                    ...
                    logger.info(
                        "epoch=%d/%d compare_base=%s train_ch_%s=%s val_ch_%s=%s train_loss=%.6e val_loss=%.6e",
                        epoch,
                        total_epochs,
                        compare_base_loss_name,
                        compare_base_loss_name,
