...
renders a histogram with a Gaussian-KDE curve scaled to the histogram counts. Figures
have no title by default (pass --title to set one) and are saved to the inference file's
folder by default (override with --output-dir).

Supported losses: mae, mse, nmae, nmse (aliases l1, l2, nl1, nl2). NMAE/NMSE use the
eps-stabilized denominators from per_sample_loss.py (defaults 1e-6 / 1e-12).
"""
...
def fit_curve(values: np.ndarray, bin_edges: np.ndarray, n_samples_for_kde: int, seed: int):
    """Return (x, y) of a Gaussian-KDE density scaled to linear histogram counts, or None.

    KDE is fit on a random subsample (for speed) and scaled by N * bin_width so it
    overlays the count-based histogram.
    """
    ...
    ys = kde(xs) * finite.size * bin_width
    return xs, ys


def fit_curve_logx(values: np.ndarray, bin_edges: np.ndarray, n_samples_for_kde: int, seed: int):
    """Return (x, y) of a Gaussian-KDE (fit in log10 space) scaled to log-bin counts, or None.

    For log-spaced bins the per-bin width is constant in log10 units, so counts ~=
    N * kde_log(u) * dlog10. Curve x-values are returned in linear space for a log axis.
    """
    finite = values[np.isfinite(values)]
...
def main() -> None:
    ...
    for loss in losses:
        ...
        if log_x:
            ...
            n_dropped = finite.size - pos.size
            if pos.size < 2:
            ...
            if hi <= lo:
                hi = lo * 10.0
            plot_vals = pos[(pos >= lo) & (pos <= hi)]
            bin_edges = np.logspace(np.log10(lo), np.log10(hi), args.bins + 1)
            ax.hist(plot_vals, bins=bin_edges, color="steelblue", edgecolor="white", linewidth=0.3, alpha=0.85)
            ax.set_xscale("log")
            if not args.no_curve:
                curve = fit_curve_logx(plot_vals, bin_edges, args.kde_samples, args.seed)
            ...
            if n_dropped:
                ax.plot([], [], " ", label=f"{n_dropped} zero/neg excluded")
        else:
            ...
            _, bin_edges, _ = ax.hist(
                plot_vals, bins=args.bins, range=(lo, hi),
                color="steelblue", edgecolor="white", linewidth=0.3, alpha=0.85,
            )
            if not args.no_curve:
                curve = fit_curve(plot_vals, bin_edges, args.kde_samples, args.seed)
                if curve is not None:
