...
"""
Batch driver for ``generate_dispersion_dataset.py``.
...
try:
    from DIAGNOSTICS.plot_dataset_histograms import (
        save_eigenfrequency_fft_histogram,
        save_eigenfrequency_uniform_histogram,
    )
except ImportError:  # DIAGNOSTICS is local-only / gitignored
    def save_eigenfrequency_fft_histogram(*_args, **_kwargs):
        raise RuntimeError("DIAGNOSTICS/plot_dataset_histograms.py not available")

    def save_eigenfrequency_uniform_histogram(*_args, **_kwargs):
        raise RuntimeError("DIAGNOSTICS/plot_dataset_histograms.py not available")
...
def _run_one_batch(
    ...
    cmd = [
        sys.executable,
        str(repo_root / "generate_dispersion_dataset.py"),
        "--n-struct",
        str(n_struct),
        "--rng-seed-offset",
        str(seed_offset),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run generate_dispersion_dataset.py in seed-shifted batches.")
