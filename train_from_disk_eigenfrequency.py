"""
Disk-backed I3O1 training: single output channel from eigenfrequency_*_full.pt at reduced_indices.
Standalone entrypoint; same training flow as train_from_disk.py except output channels and targets.
"""
...
class EigenOnlyShardedDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    """Disk-backed dataset: inputs.pt; single channel from eigenfrequency_*_full.pt via reduced_indices."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Disk-backed I3O1 (eigenfrequency-only) training with local file logging.")
...
def build_run_name(args: argparse.Namespace) -> str:
    ...
    return (
        f"NO_I3O1_EIG_BCF16_{loss_tag}_HC{args.hidden_channels}_"
...
def setup_logger(log_path: Path) -> logging.Logger:
    logger = logging.getLogger("train_from_disk_eigenfrequency")
...
def main() -> None:
    ...
    try:
        ...
        train_ds = EigenOnlyShardedDataset(train_shards, args.eigen_ch0_encoding)
        test_ds = EigenOnlyShardedDataset(test_shards, args.eigen_ch0_encoding)
        ...
        params: dict[str, Any] = {
            ...
            "target_source": "eigenfrequency_*_full.pt channel indexed by reduced_indices (single channel)",
            "hidden_channels": args.hidden_channels,
            "layers": args.layers,
            "modes_height": args.modes_height,
            "modes_width": args.modes_width,
