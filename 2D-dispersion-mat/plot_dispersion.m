...
% both small prescribed sets (e.g., 5) and larger datasets.
if isfield(data, 'WAVEVECTOR_DATA') && ndims(data.WAVEVECTOR_DATA) == 3
    n_struct_available = size(data.WAVEVECTOR_DATA, 3);
elseif isfield(data, 'EIGENVALUE_DATA') && ndims(data.EIGENVALUE_DATA) == 3
    n_struct_available = size(data.EIGENVALUE_DATA, 3);
elseif isfield(data, 'N_struct')
    n_struct_available = double(data.N_struct(1));
else
    n_struct_available = 1;
end
struct_idxs = 1:n_struct_available;
...
%     ax.CLim = []; % optional: set color limits if desired
    ...
    if ~isempty(force_plot_symmetry_type)
        sym_type = force_plot_symmetry_type;
    end
