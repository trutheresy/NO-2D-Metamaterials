...
eigenvector_dtype = 'double'; % NEW: Save eigenvectors as either 'single' or 'double' datatype (do *NOT* use 'half', it is not supported)
...
N_struct = 10; % Determines how many designs will be generated
...
design_params.design_options = struct('kernel','periodic - not squared','sigma_f',1,'sigma_l',1,'symmetry_type','p4mm','N_value',inf);
...
WAVEVECTOR_DATA = zeros(size(const.wavevectors,1),2,N_struct);
% EIGENVALUE_DATA = zeros(prod(const.N_wv),const.N_eig,N_struct);
EIGENVALUE_DATA = zeros(size(const.wavevectors,1),const.N_eig,N_struct);
...
EIGENVECTOR_DATA = zeros(N_dof,size(const.wavevectors,1),const.N_eig,N_struct,const.eigenvector_dtype); % NEW: Save eigenvectors as either 'single' or 'double' datatype
...
K_DATA = cell(N_struct,1);
M_DATA = cell(N_struct,1);
...
%% Save the results
vars_to_save = {'WAVEVECTOR_DATA','EIGENVALUE_DATA','CONSTITUTIVE_DATA','K_DATA','M_DATA','T_DATA','const','design_params','designs','N_struct','imag_tol','rng_seed_offset'};
if isSaveEigenvectors
