...
for struct_idx = struct_idxs
    ...
    disp(size(aux_data.WAVEVECTOR_DATA))
    wavevectors = aux_data.WAVEVECTOR_DATA(:,:,struct_idx);

    disp('size(data.EIGENVALUE_DATA)')
    disp(size(aux_data.EIGENVALUE_DATA))
    frequencies = aux_data.EIGENVALUE_DATA(:,:,struct_idx);
    ...
    [wavevectors_contour, contour_info] = get_IBZ_contour_wavevectors(N_k,aux_data.const.a,'p4mm'); % NOTE: I used symmetry='p4mm' here. If you have any unit cells that are not 'p4mm', then we can't use this contour
    ...
    %% Plot the dispersion relation (reconstructed with eigenvector) on the IBZ contour
